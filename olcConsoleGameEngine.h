...
public:
	...
	{	
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&::GameThread, this);
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&::AudioThread, this);
	...
	{
		((*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> ::m_bAtomActive(false);
